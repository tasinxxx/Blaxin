...
function StepRow({ step, index }: { step: { id: string; description: string; status: string; verification?: string; error?: string }; index: number }) {
  const statusLabel =
  ...
  return (
    <div className="jh-mission-step" title={`${step.description}${step.error ? ` — ${step.error}` : ''}`}>
      ...
      <span className="jh-mission-step-d">{step.description.length > 34 ? `${step.description.slice(0, 34)}…` : step.description}</span>
